...
const GenerateReports = () => {
  ...
  useEffect(() => {
    const fetchEmployeeSalaryData = async () => {
      try {
        const response = await fetch("http://localhost:5555/EmployeeSalary");
    ...
    const fetchEmployeeAttendanceData = async () => {
      try {
        const response = await fetch("http://localhost:5555/EmployeeAttendance");
        const data = await response.json();
  ...
  const generatePDF = () => {
    ...
    const head = selectedReport === "salary"
      ...
      : [["Employee Name", "NIC", "Date", "Check-In", "Check-Out", "Total Hours"]];
    ...
    const body = data.map((item) => selectedReport === "salary"
      ...
      : [item.employeeName, item.NIC, item.date, item.checkIn, item.checkOut, item.totalHours]);
  ...
  const generateCSV = () => {
    ...
    const csvData = selectedReport === "salary"
      ...
      : [["Employee Name", "NIC", "Date", "Check-In", "Check-Out", "Total Hours"],
          ...data.map((item) => [item.employeeName, item.NIC, item.date, item.checkIn, item.checkOut, item.totalHours])];
              ...
              <tbody>
                {attendanceReportData.map((attendance) => (
                  <tr key={attendance._id} className="text-center">
                    ...
                    <td className="py-2 px-4">{attendance.checkIn}</td>
                    <td className="py-2 px-4">{attendance.checkOut}</td>
                    <td className="py-2 px-4">{attendance.totalHours}</td>
                  </tr>
                ))}
              </tbody>
            </table>
